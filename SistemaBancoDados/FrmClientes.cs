    ...
    public partial class FrmClientes : Form
    {
        ...
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            if (txtNomeFil.Text.Trim() != string.Empty)
            {
                try
                {
                    DataTable dt = dbInstance.ExecutarConsulta($"SELECT * FROM customers WHERE CompanyName LIKE '%{txtNomeFil.Text.Trim()}%' ");
            ...
            else
            {
                try
                {
                    DataTable dt = dbInstance.ExecutarConsulta("SELECT * FROM customers");
                    dataGridView1.DataSource = dt;
                }
                catch (Exception ex)
                {
